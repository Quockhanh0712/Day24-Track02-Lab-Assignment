...
@pytest.fixture
def sample_df():
    # Sử dụng dtype=str để không bị mất các số 0 ở đầu CCCD hoặc số điện thoại
...
class TestAnonymization:
    ...
    def test_pii_not_in_output(self, anonymizer, sample_df):
        ...
        anon_cccds = df_anon["cccd"].astype(str).tolist()
    ...
    def test_non_pii_columns_unchanged(self, anonymizer, sample_df):
        ...
        # Convert numeric columns to numeric for comparison since raw data was read as string
        df_anon = anonymizer.anonymize_dataframe(sample_df)
        pd.testing.assert_series_equal(df_anon["benh"], sample_df["benh"])
        pd.testing.assert_series_equal(
            df_anon["ket_qua_xet_nghiem"].astype(float), 
            sample_df["ket_qua_xet_nghiem"].astype(float)
        )
        pd.testing.assert_series_equal(df_anon["patient_id"], sample_df["patient_id"])
